...
module.exports = {
  ...
  handleResponse: function (req, res) {
    if (isValidUrl(req.params.urlParam)) {
      urlExists(req.params.urlParam).then(function (err, data) {
        if (!err) {
          !data.exists
            ? shortenUrl(req.params.urlParam, data.maxCount + 1).then(function (err, data) {
              !err ? res.json(data) : res.json(failedMsg);
            })
            : res.json({ original_url: data.original_url, short_url: short_url });
        } else {
          res.json(failedMsg);
        }
      });
